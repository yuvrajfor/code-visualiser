    ...
    <div className={`lab-shell visual-theme-${visualTheme} min-h-screen overflow-x-hidden text-[#f7f0e8]`} data-visual-theme={visualTheme}>
      ...
      {activeView === "studio" && currentStep && (
        <main className="lab-grid studio-learning-frame mx-auto w-full max-w-7xl px-5 py-7 md:px-8">
          ...
          <section className="execution-workspace-grid grid gap-6 lg:grid-cols-[1.05fr_.95fr]">
              ...
              <div className="mt-5">
                <div><div key={`scene-${currentStep.step}-${currentStep.story.kind}-${visualTheme}`} data-story-scene data-story-step={currentStep.step}><RealWorldScene story={currentStep.story} visualTheme={visualTheme} routeState={currentStep.routeState} /></div>
                  <div key={`visual-focus-${currentStep.step}-${visualTheme}`} className="story-scene-brief execution-state-detail-enter mt-4" data-story-visual-focus>
                    <div data-story-visual-focus className="flex min-w-0 items-start gap-3"><div className="story-scene-brief-icon"><SceneKindIcon kind={currentStep.story.kind} className="h-4 w-4" /></div><div className="min-w-0"><p className="story-scene-brief-label">Scene focus</p><p className="mt-1 text-xs leading-5 text-[#d5def2]">{currentStep.story.visualFocus ?? `Follow the ${currentStep.story.objectLabel} as this ${sceneStyles[currentStep.story.kind].label} changes.`}</p></div></div>
                ...
                <details className="cinematic-details mt-4" data-cinematic-details>
                  <summary><span><span className="font-black">Scene detail</span><span className="ml-2 text-[#8fa4b8]">Optional Python illustration</span></span><span className="cinematic-details-hint">Open only when you want more visual detail</span></summary>
            ...
            <div className="lab-surface story-explanation-panel rounded-[28px] p-5 md:p-6">
              ...
              <div key={`active-code-${currentStep.step}-${visualTheme}`} data-active-code-line={currentStep.line} className="execution-state-detail-enter mt-5 rounded-2xl border border-amber-300/20 bg-amber-300/5 p-4"><div className="flex items-center justify-between gap-3"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-amber-200">The code says</p><span className="rounded-full border border-amber-200/20 bg-amber-100/10 px-2 py-1 text-[10px] font-bold text-amber-100">Active line</span></div><code className="mt-2 block whitespace-pre-wrap break-words font-mono text-xs leading-6 text-[#f9e9d5]">{currentStep.code}</code></div>
