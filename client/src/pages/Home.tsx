...
export default function Home() {
  ...
  const [landingWorkspace, setLandingWorkspace] = useState<LearningWorkspace>("overview");
  ...
  return (
    <div className={`lab-shell visual-theme-${visualTheme} min-h-screen overflow-x-hidden text-[#f7f0e8]`}>
      <header className="sticky top-0 z-50 border-b px-5 py-3 backdrop-blur-xl md:px-8">
      ...
      {activeView === "landing" && (
        <main className={`lab-grid landing-mode-${landingWorkspace} mx-auto w-full max-w-7xl px-5 py-10 md:px-8 md:py-14`} data-learning-workspace={landingWorkspace}>
          ...
          <section data-detailed-workspaces className="mt-10 grid gap-6 lg:grid-cols-[1.3fr_.7fr]">
            <div className="lab-surface relative overflow-hidden rounded-[28px] p-6 md:p-8">
              ...
              <div data-code-only className="relative flex flex-wrap items-center justify-between gap-4 border-b border-white/10 pb-5">
                <div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-xl border border-indigo-300/20 bg-indigo-300/10 text-indigo-100"><Code2 className="h-5 w-5" /></div><div><p className="lab-kicker">Code studio</p><h2 className="mt-1 text-sm font-bold text-white">Start with a code idea</h2><p className="text-xs text-[#a89787]">Paste a small program or choose a guided learning scene.</p></div></div>
              ...
              <div className="relative mt-6 space-y-5">
                <div>
                  <div data-code-only className="mb-2 flex items-center justify-between gap-3"><Label className="block text-[11px] font-bold uppercase tracking-wider text-[#bba797]">Choose your visual world</Label><span className="text-[10px] text-[#8f7c6d]">You can change this later</span></div>
                  <div data-code-only className="grid grid-cols-3 gap-2">{visualThemes.map((theme) => <button key={theme.id} onClick={() => setVisualTheme(theme.id)} aria-pressed={visualTheme === theme.id} className={`rounded-2xl border p-3 text-left transition ${visualTheme === theme.id ? "border-amber-300/65 bg-amber-300/10 shadow-[0_0_18px_rgba(245,158,11,0.14)]" : "border-white/10 bg-[#0c0806] hover:border-[#694b35]"}`}><span className="text-lg">{theme.icon}</span><span className="ml-1.5 text-xs font-bold text-white">{theme.shortLabel}</span></button>)}</div>
                  <p data-code-only className="mt-2 text-[11px] leading-relaxed text-[#a89787]">{activeTheme.description}</p>
                </div>
                <div>
                  <div data-code-only className="mb-2 flex items-center justify-between gap-3"><Label className="block text-[11px] font-bold uppercase tracking-wider text-[#bba797]">Try a ready-made everyday example</Label><span className="text-[10px] text-[#8f7c6d]">or write your own below</span></div>
                  <div data-code-only className="grid gap-2 sm:grid-cols-2">{learningPresets.map((preset) => <button key={preset.name} onClick={() => { setUserProblem(preset.problem); setSelectedLang(preset.language); setUserCode(preset.code); setSelectedWalkthrough(preset.walkthrough ?? null); toast.success(`${preset.name} example loaded.`); }} className="group rounded-2xl border border-white/10 bg-[#0c0806] p-3 text-left transition hover:-translate-y-0.5 hover:border-amber-300/45 hover:bg-[#1b110b]"><span className="text-lg">{preset.icon}</span><span className="ml-2 text-xs font-bold text-white">{preset.name}</span><span className="mt-1 block text-[10px] leading-relaxed text-[#a89787]">{preset.description}</span></button>)}</div>
                </div>
              ...
              <div data-code-only className="relative mt-6 flex justify-end"><Button onClick={startVisualStory} className="h-12 rounded-xl bg-gradient-to-r from-[#ffbd7d] via-[#e07834] to-[#c34d20] px-6 text-sm font-black text-[#170b06] shadow-[0_0_32px_rgba(229,155,99,0.33)] hover:scale-[1.01]">Turn it into a visual story <ArrowRight className="ml-2 h-4 w-4" /></Button></div>
            </div>

            <aside className="lab-surface rounded-[28px] p-6">
              <div className="flex items-center gap-2"><BookOpen className="h-4 w-4 text-indigo-200" /><div><p className="lab-kicker">Learning kit</p><h2 className="mt-1 text-sm font-bold text-white">Visual building blocks</h2></div></div>
              <p className="mt-3 text-xs leading-relaxed text-[#a89787]">Every technical idea gets a physical metaphor you can return to while you learn.</p>
      ...
      {activeView === "studio" && currentStep && (
        <main className="lab-grid mx-auto w-full max-w-7xl px-5 py-7 md:px-8">
          <div className="lab-surface mb-6 flex flex-col justify-between gap-4 rounded-3xl p-5 md:flex-row md:items-center">
            <div><p className="lab-kicker">Code story workspace</p><h1 className="mt-1 text-xl font-extrabold tracking-tight text-white">{userProblem || "Your code story"}</h1><p className="mt-1 text-xs text-[#a89787]">Scene, plain-English explanation, and a clickable learning timeline stay together.</p></div>
          ...
          <section className="grid gap-6 lg:grid-cols-[1.05fr_.95fr]">
            <div className="lab-surface rounded-[28px] p-5 md:p-6">
